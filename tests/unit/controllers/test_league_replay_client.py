...
async def test_league_replay_client_initialization():
    ...
    assert client.api_client.base_url == "https://127.0.0.1:2999"
...
async def test_get_playback_properties():
    ...
    with patch.object(client.api_client, 'request') as mock_req:
        mock_req.return_value = mock_response
        result = await client.get_playback_properties()
        assert result["time"] == 100.0
        mock_req.assert_called_once_with("/replay/playback")
...
async def test_get_recording_properties():
    ...
    with patch.object(client.api_client, 'request') as mock_req:
        ...
        mock_req.assert_called_once_with("/replay/recording")
...
async def test_get_render_properties():
    ...
    with patch.object(client.api_client, 'request') as mock_req:
        ...
        mock_req.assert_called_once_with("/replay/render")
...
async def test_post_playback_properties():
    ...
    with patch.object(client.api_client, 'request') as mock_req:
        ...
        mock_req.assert_called_once_with("/replay/playback", method="POST", body=options)
...
async def test_get_process_id_from_api():
    ...
    with patch.object(client.api_client, 'request') as mock_req:
        ...
        mock_req.assert_called_once_with("/replay/game")
...
async def test_get_all_game_data():
    ...
    with patch.object(client.api_client, 'request') as mock_req:
        ...
        mock_req.assert_called_once_with("/liveclientdata/allgamedata")