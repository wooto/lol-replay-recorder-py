...
class IniEditor:
    ...
    def update(self, path: str, value: Any) -> None:
        ...
        For INI files, path should be in format "section.key".
        """
        parts = path.split(".", 1)  # Split into section and key
        if len(parts) != 2:
            raise ConfigError(f"Invalid INI path format: {path}. Expected 'section.key'")
